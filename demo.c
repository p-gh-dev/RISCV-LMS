 ...
 * SHA-256 is default, SHA-3 support added
 *
 *   demo genkey keyname
 ...
 *   demo genkey keyname  15/4,10/8:2000    (sha3/15/4:2000 for SHA-3)
...
//Define hash counters from hash.c
...
static int get_hash_type(const char **p) {
    int hash_type = 0; // Default: SHA-2
    if (strncmp(*p, "sha3/", 5) == 0) {
        *p += 5; // Advance pointer past "sha3/"
        hash_type = 1;
    } else if (strncmp(*p, "sha2/", 5) == 0) {
        *p += 5; // Advance pointer past "sha2/"
        hash_type = 0;
    }
    // If no prefix, pointer is unchanged and SHA-2 is default
    return hash_type;
}
...
static int parse_parm_set( int *levels, param_set_t *lm_array,
    ...
    /*Get hash type*/
    int hash_type = get_hash_type( &parm_set );
    ...
    for (i=0;; i++) {
        ...
        param_set_t lm;
        switch (h) {
        case 5:  lm = (hash_type == 0) ? LMS_SHA256_M32_H5  : LMS_SHAKE_M32_H5;  break;
        case 10: lm = (hash_type == 0) ? LMS_SHA256_M32_H10 : LMS_SHAKE_M32_H10; break;
        case 15: lm = (hash_type == 0) ? LMS_SHA256_M32_H15 : LMS_SHAKE_M32_H15; break;
        case 20: lm = (hash_type == 0) ? LMS_SHA256_M32_H20 : LMS_SHAKE_M32_H20; break;
        case 25: lm = (hash_type == 0) ? LMS_SHA256_M32_H25 : LMS_SHAKE_M32_H25; break;
        ...
        if (*parm_set == '/') {
            ...
            switch (w) {
            case 1: ots = (hash_type == 0) ? LMOTS_SHA256_N32_W1 : LMOTS_SHAKE_N32_W1; break;
            case 2: ots = (hash_type == 0) ? LMOTS_SHA256_N32_W2 : LMOTS_SHAKE_N32_W2; break;
            case 4: ots = (hash_type == 0) ? LMOTS_SHA256_N32_W4 : LMOTS_SHAKE_N32_W4; break;
            case 8: ots = (hash_type == 0) ? LMOTS_SHA256_N32_W8 : LMOTS_SHAKE_N32_W8; break;
...
static void list_parameter_set(int levels, const param_set_t *lm_array,
    ...
    for (i=0; i<levels; i++) {
        ...
        switch (lm_array[i]) {
            case LMS_SHAKE_M32_H5: case LMS_SHAKE_M32_H10:
            case LMS_SHAKE_M32_H15: case LMS_SHAKE_M32_H20:
            case LMS_SHAKE_M32_H25:
                hash_name = "SHAKE-256";
                break;
        }
        printf( "Level %d: hash function = %s; ", i, hash_name );
        int h = 0;
        switch (lm_array[i]) {
        case LMS_SHA256_M32_H5 : case LMS_SHAKE_M32_H5 : h =  5; break;
        case LMS_SHA256_M32_H10: case LMS_SHAKE_M32_H10: h = 10; break;
        case LMS_SHA256_M32_H15: case LMS_SHAKE_M32_H15: h = 15; break;
        case LMS_SHA256_M32_H20: case LMS_SHAKE_M32_H20: h = 20; break;
        case LMS_SHA256_M32_H25: case LMS_SHAKE_M32_H25: h = 25; break;
        }
        printf( "%d level Merkle tree; ", h );
        int w = 0;
        switch (ots_array[i]) {
        case LMOTS_SHA256_N32_W1: case LMOTS_SHAKE_N32_W1: w = 1; break;
        case LMOTS_SHA256_N32_W2: case LMOTS_SHAKE_N32_W2: w = 2; break;
        case LMOTS_SHA256_N32_W4: case LMOTS_SHAKE_N32_W4: w = 4; break;
        case LMOTS_SHA256_N32_W8: case LMOTS_SHAKE_N32_W8: w = 8; break;
        }
...
int main(int argc, char **argv) {
    ...
    printf( "Hash function calls: %lu\n", hash_calls);
    printf( "Incremental hash inits: %lu\n", inc_hash_init);
    printf( "Incremental hash updates: %lu\n", inc_hash_update);
    printf( "Incremental hash finalizes: %lu\n", inc_hash_finalize);
    return 0;
}
