...
sha3_return_t
bh_sha3_Init(void *priv, unsigned bitSize) {
...
void
bh_sha3_Init256(void *priv)
{
    bh_sha3_Init(priv, 256);
}

void
bh_sha3_Init384(void *priv)
{
    bh_sha3_Init(priv, 384);
}

void
bh_sha3_Init512(void *priv)
{
    bh_sha3_Init(priv, 512);
}

enum SHA3_FLAGS
bh_sha3_SetFlags(void *priv, enum SHA3_FLAGS flags)
{
    sha3_context *ctx = (sha3_context *) priv;
...
void
bh_sha3_Update(void *priv, void const *bufIn, size_t len)
{
    sha3_context *ctx = (sha3_context *) priv;
...
sha3_return_t bh_sha3_HashBuffer( unsigned bitSize, enum SHA3_FLAGS flags, const void *in, unsigned inBytes, void *out, unsigned outBytes ) {
    sha3_return_t err;
    sha3_context c;

    err = bh_sha3_Init(&c, bitSize);
    if( err != SHA3_RETURN_OK )
        return err;
    if( bh_sha3_SetFlags(&c, flags) != flags ) {
        return SHA3_RETURN_BAD_PARAMS;
    }
    bh_sha3_Update(&c, in, inBytes);
    const void *h = bh_sha3_Finalize(&c);
