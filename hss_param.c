...
bool hss_compress_param_set( unsigned char *compressed,
    ...
    for (i=0; i<levels; i++) {
        ...
        if (a > 0x13 || b > 0x13) return false;
            /* Make sure the parm sets are supported */
        switch (a) {
        case LMS_SHA256_M32_H5: case LMS_SHA256_M32_H10:
        case LMS_SHA256_M32_H15: case LMS_SHA256_M32_H20:
...
bool hss_get_parameter_set( unsigned *levels,
    ...
    for (level=0; level < MAX_HSS_LEVELS; level++) {
        ...
        unsigned char lm = private_key[PRIVATE_KEY_PARAM_SET + 2*level];
        if (lm == PARM_SET_END) break;
        unsigned char ots = private_key[PRIVATE_KEY_PARAM_SET + 2*level + 1];
            /* Make sure both are supported */
            /* While we're here, add up the total Merkle height */
        switch (lm) {
        case LMS_SHA256_M32_H5 : case LMS_SHAKE_M32_H5 : total_height += 5; break;
        case LMS_SHA256_M32_H10: case LMS_SHAKE_M32_H10: total_height += 10; break;
        case LMS_SHA256_M32_H15: case LMS_SHAKE_M32_H15: total_height += 15; break;
        case LMS_SHA256_M32_H20: case LMS_SHAKE_M32_H20: total_height += 20; break;
        case LMS_SHA256_M32_H25: case LMS_SHAKE_M32_H25: total_height += 25; break;
        default: goto failed;
        }
        switch (ots) {
        case LMOTS_SHA256_N32_W1: case LMOTS_SHAKE_N32_W1:
        case LMOTS_SHA256_N32_W2: case LMOTS_SHAKE_N32_W2:
        case LMOTS_SHA256_N32_W4: case LMOTS_SHAKE_N32_W4:
        case LMOTS_SHA256_N32_W8: case LMOTS_SHAKE_N32_W8:
            break;
        default: goto failed;
        }
        lm_type[level] = lm;
